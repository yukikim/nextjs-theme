    ...
    <div id="features">
      <Container>
        <div className="md:w-2/3 lg:w-1/2">
          ...
          <h2 className="my-8 text-2xl font-bold text-on-tertiary md:text-4xl">A technology-first approach to payments and finance</h2>
        ...
        <div className="mt-16 grid divide-x divide-y divide-gray-100 overflow-hidden rounded-3xl border border-gray-100 text-gray-600 sm:grid-cols-2 lg:grid-cols-4 lg:divide-y-0 xl:grid-cols-4">
          {["4341139", "4341134", "4341160", "4341025"].map((icon, index) => (
            <div
              key={icon}
              className="group relative transition hover:z-[1] hover:shadow-2xl hover:shadow-gray-600/10 bg-background"
            >
              <div className={`relative space-y-8 py-12 p-8 ${index === 3 ? "transition duration-300 group-hover:bg-white dark:group-hover:bg-gray-800" : ""}`}>
                <Image src={`https://cdn-icons-png.flaticon.com/512/4341/${icon}.png`} className="w-12" width={48} height={48} alt="feature icon" />
                <div className="space-y-2">
                  <h5 className="text-xl font-semibold text-on-background transition group-hover:text-secondary">
                    {index === 3 ? "More features" : `${["First", "Second", "Third"][index]} feature`}
                  </h5>
                  <p className="text-on-background">Neque Dolor, fugiat non cum doloribus aperiam voluptates nostrum.</p>
                </div>
