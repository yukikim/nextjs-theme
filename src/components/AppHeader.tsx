  ...
  return (
    <header className="">
      <nav
        id="nav"
        data-state={navState}
        className="fixed group z-10 w-full bg-background/90"
      >
        <Container>
          <div className="relative flex flex-wrap items-center justify-between gap-6 py-3 md:gap-0 md:py-4">
            <div className="relative z-20 flex w-full justify-between md:px-0 lg:w-fit">
              <Link href="#home" aria-label="logo" className="flex items-center space-x-2">
                {/* <span className="font-racing-sans-one text-2xl font-bold text-primary">Adlib Go</span> */}
                <div id="logo" className="w-22 h-8 bg-[url('/images/main_logo.svg')] bg-center bg-no-repeat"></div>
              </Link>
            ...
            <div
              id="navLayer"
              aria-hidden="true"
              className="fixed inset-0 z-10 h-screen w-screen origin-bottom scale-y-0 bg-white/70 backdrop-blur-2xl transition duration-500 group-data-[state=active]:origin-top group-data-[state=active]:scale-y-100 lg:hidden"
            />
            <div
              id="navlinks"
            ...
            >
              <div className="w-full text-on-background lg:w-auto lg:pr-4 lg:pt-0">
                <div id="links-group" className="flex flex-col gap-6 tracking-wide lg:flex-row lg:gap-0 lg:text-sm">
                  {links.map((link) => (
                    <a
                      key={link.label}
                      href={link.to}
                      className="hover:text-gray-500 block transition md:px-4 font-semibold"
